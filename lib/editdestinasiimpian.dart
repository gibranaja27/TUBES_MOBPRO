...
class EditDestinasiImpian extends StatefulWidget {
  final Map ListData;

  EditDestinasiImpian({Key? key, required this.ListData}) : super(key: key);

  @override
  State<EditDestinasiImpian> createState() => _EditDestinasiImpianState();
}

class _EditDestinasiImpianState extends State<EditDestinasiImpian> {
  ...
  Future _update() async {
    final respone = await http.post(
        Uri.parse("http://192.168.221.236/flutterapi/crudtravelkuy/edit.php"),
        body: {
          "id" : id.text,
  ...
  Widget build(BuildContext context) {
    ...
    tempat_destinasi.text = widget.ListData['tempat_destinasi'];
    budget.text = widget.ListData['budget'];
    tanggal_mulai.text = widget.ListData['tanggal_mulai'];
    catatan_tambahan.text = widget.ListData['catatan_tambahan'];
    return Scaffold(
      appBar: AppBar(
        title: Text("Update Destinasi Impian",
            style: TextStyle(
                color: Colors.white,
                fontSize: 20.0,
                    ...
                    onPressed: () {
                      if (formkey.currentState!.validate()) {
                        ...
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: ((context) => DestinasiImpian())));
                      }
                    },
                    child: Text("Update"))
              ],
